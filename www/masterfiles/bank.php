...
<title>Modularfusion Inc - Bank</title>
...
<script type="text/template" id="modal-bank-tpl">
	<form id="frm-mdl-item" name="frm-mdl-item" class="table-model" data-table="item" action="" method="post">	
    	<table cellpadding="5px" cellspacing="0" border="0">
        	<tbody>
				...
				<tr>
                    <td><label for="acctno">Account No:</label></td>
                    <td><input type="text" name="acctno" id="acctno" maxlength="50" class="m-input" value="<%= acctno %>" required></td>
                </tr>
        	</tbody>
    	</table>
 	</form>
...
<script type="text/template" id="modal-bank-readonly-tpl">
    ...
    <table cellpadding="5px" cellspacing="0" border="0">
            <tbody>
                ...
                <tr>
                    <td><label for="acctno">Account No: </label></td>
                    <td><input type="text" name="acctno" id="acctno" maxlength="20" class="m-input"  value="<%= acctno %>"  readonly>     </td>
                </tr>
...
var dtc = new DatatableCollection();
...
$(document).ready(function(e) {
	...
	var bankModal = new BankModal({ model: bank });
	bankModal.render();
	//console.log(itemcatModalView.el);
	var bankDataGridView = new DataGridView({model: bank });

	$('#tlbr-new').on('click', function(){
		//$(".modal .modal-title").text('Add');
		bankModal.modalTitle.text('Add Record');
		bankModal.clearForm();
	...
	oTable = $('.tb-data').dataTable( {
		...
		"bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": "../api/datatables/bank",
	//	"sAjaxSource": "../www/test/datatable_test.php"
		...
		"aoColumns": [
            ...
            { "mData": "acctno",  "sTitle": "Account No" }
			],
