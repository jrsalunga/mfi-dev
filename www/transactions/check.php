    	...
    	<table cellpadding="5px" cellspacing="0" border="0" width="100%" >
        	<tbody>
				...
				<tr>
                    ...
                    <td><textarea type="text" name="due" id="due" style="width: 145px; height: 50px;"></textarea></td>
                    ...
                    success: function( data ) {
                        response( $.map( data, function( item ) {
                            return {
                                label: item.refno,
                                value: item.refno,
								amount: item.balance || item.totamount,
								id: item.id
                            }
                        }));
                    }
...
$(document).ready(function(e) {
	...
	var parentChildModal = new ParentChildModal({model: cvhdr, child1: cvapvdtls, child2: cvchkdtls, settings: modalData});
    parentChildModal.render();
	//console.log(parentChildModal);
	
	var cvapvdtlsView = new CvapvdtlsView({collection: cvapvdtls});
  	cvapvdtlsView.render();
	
	var cvchkdtlsView = new CvchkdtlsView({collection: cvchkdtls});
  	cvchkdtlsView.render();
	
	
	var searchApvhdrView = new SearchApvhdrView({model: cvhdr, collection: cvapvdtls});
	searchApvhdrView.render();
	
	var searchChkdtlView = new SearchChkdtlView({model: cvhdr, collection: cvchkdtls});
	searchChkdtlView.render();
	
	var cvhdrDataGridView = new DataGridView2({model: cvhdr, settings: modalData});
	/*
	...
	*/
	...
	oTable = $('.tb-data').dataTable( {
		...
		"aoColumns": [
			...
			/*
			{ "mData": "date",  "sTitle": "Due",
				"mRender": function ( data, type, apvhdr ) {
					console.log(apvhdr);
							return moment(data).add('days', apvhdr.term).format("YYYY-MM-DD");
			...
			*/
			...
			{ "mData": "totapvamt",  "sTitle": "APV Total" },
			{ "mData": "totchkamt",  "sTitle": "Check Total" },
...
<div id="container">
        	...
        	<nav class="main-nav">
                ...
                <div id="menu1" class="nav active">
                    ...
                    <ul class="fd">
                        ...
                        <li class="active"><a href="check">Check</a></li>
	  	...
	  	<div role="content" class="animated">
        	...
        	<section>
                ...
                <div class="tb-data-container">
                	<table id="grid-cvhdr" class="tb-data" cellpadding="0" cellspacing="0" width="100%" >
		                       <!-- <thead>
		                          <tr>
 ...
 <div class="modal fade" id="mdl-frm-cvhdr" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
        	<div class="modal-parent-child">
            	<div class="tab-cvhdr"></div>
                <div class="tab-cvapvdtl" style="border: 0px solid #ccc; width:100%; margin-top:20px;">
                	<table id="child-cvapvdtl" data-table="cvapvdtl" class="modal-tb-detail" width="100%" cellspacing="0" cellpadding="0" border="0">
                    	<thead>
                        	<tr><th>APV Code</th><th>Amount</th></tr>
                        </thead>
                    	<tbody class="items-tbody">
                    	</tbody>
                    </table>
                    <form id="search-apvhdr" class="modal-table-detail">
                        <input type="hidden" name="id" id="id">
                        <input type="hidden" name="apvhdrid" id="apvhdrid">
                        <input type="text" class="search-detail" placeholder="Search APV">
                        <a href="#" data-toggle="popover">?</a>
                        <input type="text" name="amount" id="amount" placeholder="Amount" class="currency">
                ...
                <div class="tab-cvchkdtl" style="border: 0px solid #ccc; width: 100%; margin-top:20px;">
                	<table id="child-cvchkdtl" data-table="cvchkdtl" class="modal-tb-detail" width="100%" cellspacing="0" cellpadding="0" border="0">
                    	<thead>
                        	<tr><th>Bank</th><th>Check No</th><th>Check Date</th><th>Amount</th></tr>
                        </thead>
                    	<tbody class="items-tbody">
                    	</tbody>
                    </table>
                    <form id="search-chkdtl" class="modal-table-detail">
                        <input type="hidden" name="id" id="id">
                        <select name="bankacctid" id="bankacctid" style="width:120px;">
                        <?php
                            ...
                            foreach( $banks as  $bank) {                        
                               echo "<option value=\"".strtolower($bank->id)."\">". $bank->code ." - ". $bank->acctno ."</option>";
                            }  
						...
						<input type="text" name="checkdate" id="checkdate" placeholder="Check Date" style="width:120px;" value="<?=iso_date()?>">
                        <input type="text" name="amount" id="amount" placeholder="Amount" class="currency">
                        <button type="button" id="mdl-detail-save-item" class="btn btn-primary btn-sm">Add</button>
                        <button type="button" id="mdl-detail-cancel-item" class="btn btn-default btn-sm">Cancel</button>
                    </form>
            	...
            	<!--
                <ul id="cvhdrTab" class="nav nav-tabs">
                    <li class="active">
                    	<a data-toggle="tab" href="#tab-general">General</a>
                    </li>
                    <li class="">
                    	<a data-toggle="tab" href="#tab-cvapvdtl">APV Details</a>
                    </li>
                    <li class="">
                    	<a data-toggle="tab" href="#tab-cvchkdtl">Check Details</a>
                    </li>
                </ul>
                <div class="tab-content">
                    ...
                    <div id="tab-cvapvdtl" class="tab-pane">
                    fsadfasdfa
                    </div>
                    <div id="tab-cvchkdtl" class="tab-pane">
                    fsadfafasdfa
                    </div>
                </div>
                -->
